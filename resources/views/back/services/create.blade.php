              ...
              <div class="card-body flex flex-col p-6">
                ...
                <form action="{{ route('back.service.store')}}" method="POST" enctype="multipart/form-data">
                  ...
                  <header class="flex mb-5 items-center border-b border-slate-100 dark:border-slate-700 pb-5 -mx-6 px-6">
                    <div class="flex-1">
                      <div class="card-title text-slate-900 dark:text-white ">Create Service</div>
                    </div>
                  </header>
                  <div class="card-text h-full space-y-4">
                    ...
                    <div class="input-area">
                        ...
                        <input id="subtitle" value="{{ old('subtitle') ? old('subtitle') : ' '}}" name="subtitle" type="text" class="form-control" placeholder="Subtile">
                        <span class="inline-block pt-2 text-red-500">{{$errors->first("subtitle")}}</span>
                    </div>
                    <div class="input-area">
                        <label for="tag" class="form-label">Tags (use comma)</label>
                        <input id="tag" value="{{ old('tag') ? old('tag') : ' '}}" name="tag" type="text" class="form-control" placeholder="Tags">
                        <span class="inline-block pt-2 text-red-500">{{$errors->first("tag")}}</span>
                    </div>
                    <div class="input-area">
                        <label for="details" class="form-label">Details</label>
                        <textarea id="trumbowyg-demo" name="details" rows="5" class="form-control" placeholder="Type Here">{{ old('details') ? old('details') : ' '}}</textarea>
