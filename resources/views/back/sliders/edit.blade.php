          ...
          <div class="grid xl:grid-cols-1 grid-cols-1 gap-6">
            <div class="card">
              <div class="card-body flex flex-col p-6">
                ...
                <form action="{{ route('back.slider.update', $slider->id)}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  ...
                  <div class="card-text h-full space-y-4">
                    <div class="input-area">
                      ...
                      <input id="getting" value="{{ old('getting') ? old('getting') : $slider->getting }}"  name="getting" type="text" class="form-control" placeholder="Getting">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("getting")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="title" class="form-label">Title</label>
                      <input id="title" value="{{ old('title') ? old('title') : $slider->title }}"  name="title" type="text" class="form-control" placeholder="Title">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("title")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="subtitle" class="form-label">Subtitle</label>
                      <input id="subtitle" value="{{ old('subtitle') ? old('subtitle') : $slider->subtitle }}" name="subtitle" type="text" class="form-control" placeholder="Heading">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("subtitle")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="description" class="form-label">Description</label>
                      <textarea id="description" name="description" rows="5" class="form-control" placeholder="Type Here">{{ old('description') ? old('description') : $slider->description }}</textarea>
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("description")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="project" class="form-label">Project</label>
                      <input id="project" value="{{old('project') ? old('project') : $slider->project }}" name="project" type="text" class="form-control" placeholder="Project">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("project")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="experience" class="form-label">Experience</label>
                      <input id="experience" value="{{old('experience') ? old('experience') : $slider->experience }}" name="experience" type="text" class="form-control" placeholder="Experience">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("experience")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="image" class="form-label">Image</label>
                      <input id="image" data-default-file="{{ asset($slider->image) }}" name="image" type="file" class="form-control dropify" placeholder="Heading">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("image")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="image_alt" class="form-label">Image_alt</label>
                      <input id="image_alt" value="{{old('image_alt') ? old('image_alt') : $slider->image_alt }}" name="image_alt" type="text" class="form-control" placeholder="Image_alt">
