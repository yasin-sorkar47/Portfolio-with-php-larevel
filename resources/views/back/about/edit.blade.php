          ...
          <div class="grid xl:grid-cols-1 grid-cols-1 gap-6">
            ...
            <div class="card">
              <div class="card-body flex flex-col p-6">
                ...
                <form action="{{ route('back.about.update', $aboutItems->slug)}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  ...
                  <div class="card-text h-full space-y-4">
                    <div class="input-area">
                      ...
                      <input id="title" value="{{ old('title') ? old('title') : $aboutItems->title }}" name="title" type="text" class="form-control" placeholder="Title">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("title")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="description" class="form-label">Description</label>
                      <textarea id="description" name="description" rows="5" class="form-control" placeholder="Type Here">{{ old('description') ? old('description') : $aboutItems->description }}</textarea>
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("description")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="image" class="form-label">Image</label>
                      <input id="image" name="image" data-default-file="{{ asset($aboutItems->image) }}" type="file" class="form-control dropify" placeholder="Heading">
