                ...
                <div class="grid xl:grid-cols-1 grid-cols-1 gap-6">
                    <div class="card">
                        <header class="card-header noborder">
                            <h4 class="card-title">All About Items</h4>
                        </header>
                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                            ...
                                            <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                                ...
                                                @forelse ($aboutItems as $aboutItem)
                                                <tr class="even:bg-slate-50 dark:even:bg-slate-700">
                                                    <td class="table-td">{{ $loop->iteration }}</td>
                                                    <td class="table-td">{{ $aboutItem->title }}</td>
                                                    <td class="table-td">{{ $aboutItem->slug }}</td>
                                                    <td class="table-td">
                                                        <img class=" w-20" src="{{ asset($aboutItem->image) }}" alt="image">
                                                    </td>
                                                    <td class="table-td">{{ $aboutItem->description }}</td>
                                                    <td class="table-td">{{ $aboutItem->created_at->format("d M Y") }}</td>
                                                    ...
                                                    <td class="table-td">
                                                        ...
                                                        <div class="flex-2 mr-3">
                                                            <div text="primary" class="btn btn-primary w-8 flex justify-center align-content-center">
                                                                ...
                                                                <a href="{{ route('back.about.edit', $aboutItem->slug)}}" class="inline-flex justify-center">
                                                        ...
                                                        <form id="form_submit" class="hidden" action="{{ route('back.about.delete')}}" method="POST">
                                                            @csrf
                                                            <input type="text" name="slug" value="{{ $aboutItem->slug }}">
                                                        </form>
                                                    </td>
                                                </tr>  
                                                @empty
