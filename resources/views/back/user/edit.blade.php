          ...
          <div class="grid xl:grid-cols-1 grid-cols-1 gap-6">
            ...
            <div class="card">
              <div class="card-body flex flex-col p-6">
                ...
                <form action="{{ route('back.user.update', $users->id )}}" method="POST" enctype="multipart/form-data">
                  @csrf
                  ...
                  <div class="card-text h-full space-y-4">
                    <div class="input-area">
                      ...
                      <input id="name" value="{{ old('name') ? old('name') : $users->name }}" name="name" type="text" class="form-control" placeholder="Name">
                      <span class="inline-block pt-2 text-red-500">{{$errors->first("name")}}</span>
                    </div>
                    <div class="input-area">
                      <label for="email" class="form-label">Email</label>
                      <input id="email" value="{{ old('email') ? old('email') : $users->email }}" name="email" type="text" class="form-control" placeholder="Email">
